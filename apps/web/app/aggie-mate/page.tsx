...
export default function AggiematePage() {
  ...
  useEffect(() => {
    function syncCurrentCourses() {
      try {
        const stored = localStorage.getItem(TRANSCRIPT_COURSES_STORAGE_KEY);
        const courses = stored ? JSON.parse(stored) : [];
        setCurrentCourses(
          Array.isArray(courses)
            ? courses.filter((course: TranscriptCourse) => course.grade === "IP")
            : []
        );
      } catch {
        setCurrentCourses([]);
    ...
    syncCurrentCourses();
    window.addEventListener("storage", syncCurrentCourses);
    window.addEventListener("focus", syncCurrentCourses);
    return () => {
      window.removeEventListener("storage", syncCurrentCourses);
